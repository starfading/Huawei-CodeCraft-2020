...
/**
 * class Sol:
 * 改用了模板编程：T->ui16/ui32/ui64
 */
template<typename T> class Sol{
public:
    struct Heap{
        ui32 num;
        T dist;
...
public:
    static void DijkWithHeap(const ui32& s, const ui8& tid, ui32* pred, ui16* len_of_pred, T* dst, ui16* sigma, double* delta, ui32* lst, bool* visit, Heap* heap) {
        int len_of_lst = 0;
        sigma[s] = 1;
        ...
        while(heap_sz) {
            ...
            for(ui32 k = l; k < r; ++k, ++e){
                ...
                const T &new_dist = u.dist + e->money;
